...
**Última actualización:** Eliminación de números mágicos y externalización de configuración  
**Estado:** Documento vivo - actualizar con cada nueva lección aprendida

## 📋 Lección Aprendida: Eliminación de Números Mágicos

### Problema Identificado
Los scripts VBS contenían múltiples "números mágicos" hardcodeados que dificultaban el mantenimiento y la configuración:

**❌ NÚMEROS MÁGICOS PROBLEMÁTICOS:**
```vbscript
' Configuraciones de Access hardcodeadas
objAccess.AutomationSecurity = 3
objAccess.Visible = True

' Tipos de módulos VBA hardcodeados
If vbComponent.Type = 1 Then  ' Módulo estándar
If vbComponent.Type = 2 Then  ' Módulo de clase

' Tipos de stream hardcodeados
...
Externalización completa de configuración mediante archivo `cli.ini` y funciones `CfgGet`:

**✅ CONFIGURACIÓN EXTERNALIZADA:**
```vbscript
' Funciones de configuración centralizadas
Function LoadConfig()
    ' Carga configuración desde cli.ini
End Function

Function CfgGet(config, key, iniPath, defaultValue)
    ' Obtiene valor con fallback a default
End Function

' Uso de configuración en lugar de números mágicos
objAccess.AutomationSecurity = CInt(CfgGet(gConfig, "ACCESS_AutomationSecurity", "ACCESS.AutomationSecurity", "3"))
objAccess.Visible = CBool(CfgGet(gConfig, "ACCESS_Visible", "ACCESS.Visible", "true"))

' Tipos de módulos configurables
If vbComponent.Type = CInt(CfgGet(gConfig, "VBA_StdModuleType", "VBA.StdModuleType", "1")) Then
If vbComponent.Type = CInt(CfgGet(gConfig, "VBA_ClassModuleType", "VBA.ClassModuleType", "2")) Then

' Tipos de stream configurables
stream.Type = CInt(CfgGet(gConfig, "VBA_StreamTypeText", "VBA.StreamTypeText", "2"))

' Delays configurables
WScript.Sleep CInt(CfgGet(gConfig, "VBA_SleepDelayMs", "VBA.SleepDelayMs", "500"))
```

### Configuración en cli.ini
```ini
[ACCESS]
Visible=true
...
[DATABASE]
Path=c:\Proyectos\cli\CONDOR.accdb
...
1. **Mantenibilidad**: Cambios de configuración sin modificar código
2. **Testabilidad**: Diferentes configuraciones para diferentes entornos
3. **Documentación**: Valores por defecto explícitos en el código
4. **Flexibilidad**: Fácil adaptación a diferentes versiones de Access/VBA
5. **Consistencia**: Misma configuración en todos los scripts del proyecto

### Archivos Actualizados

- ✅ `cli.vbs` - Script principal
- ✅ `cli_master.vbs` - Script maestro
- ✅ `test_01_open_access.vbs` - Test de apertura
- ✅ `test_02_delete_modules.vbs` - Test de eliminación
- ⏳ `test_03_import_modules.vbs` - Pendiente
- ⏳ `test_04_compile_modules.vbs` - Pendiente

### Guardarraíles para Futuros Desarrollos

1. **NUNCA hardcodear números mágicos** - Siempre usar `CfgGet`
2. **Documentar valores por defecto** - Incluir en comentarios el significado
3. **Agrupar configuraciones** - Usar secciones lógicas en `cli.ini`
4. **Validar tipos de datos** - Usar `CInt()`, `CBool()` según corresponda
5. **Mantener retrocompatibilidad** - Siempre proporcionar valores por defecto