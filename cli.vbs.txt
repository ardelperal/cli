...
Sub ShowHelp()
    ...
    WScript.Echo ""
    WScript.Echo "  rebuild-tx"
    WScript.Echo "    Reconstruccion transaccional con atomicidad en disco"
    WScript.Echo "    - Realiza toda la reconstruccion en una copia temporal (.tmp.accdb)"
    WScript.Echo "    - Solo si compila exitosamente, reemplaza el archivo original atomicamente"
    WScript.Echo "    - Si falla, el archivo original queda intacto"
    WScript.Echo "    - Recomendado para CI/CD y entornos de produccion"
    WScript.Echo ""
    WScript.Echo "  update <db_path>"
    WScript.Echo "    Actualizar modulos VBA desde src"
    WScript.Echo ""
    WScript.Echo "  lint [db_path] [src_dir] [module1] [module2] ..."
    WScript.Echo "    Valida la sintaxis de modulos VBA"
    WScript.Echo "    - Si no se especifica db_path, usa DATABASE_DefaultPath del config"
    WScript.Echo "    - Si no se especifica src_dir, usa MODULES_SrcPath del config"
    WScript.Echo "    - Si no se especifican modulos, valida todos los archivos .bas y .cls"
    WScript.Echo ""
    ...
    WScript.Echo "  cscript cli.vbs lint"
    WScript.Echo "  cscript cli.vbs lint ""C:\mi_base.accdb"" modAssert.bas"
...
Function LintModules(dbTemplatePath, srcDir, modulesArg) ' modulesArg: "" => all 
     ...
     If app Is Nothing Then WScript.Echo "lint: no se pudo abrir Access (tmp)": LintModules = False: Exit Function 
     ...
     If Not CheckVBProjectAccess(app) Then 
         ...
         WScript.Echo "lint: VBE no disponible": LintModules = False: Exit Function 
     End If 
     ...
     For i = 0 To UBound(files) 
         ...
         If Err.Number <> 0 Then 
             WScript.Echo "lint: error importando " & path & " -> " & Err.Description 
     ...
     If Not CheckMissingReferences(app) Then 
         WScript.Echo "lint: referencias rotas": CloseAccess_Robust app: LintModules = False: Exit Function 
     End If 
 
     If Not Maybe_DoCmd_CompileAndSaveAllModules(app, "lint") Then 
         WScript.Echo "lint: compilación fallida (ver log)" 
         CloseAccess_Robust app 
         LintModules = False 
         Exit Function 
     End If 
...
Sub AssertArchitecture()
    ...
    If (proc64 And off = "x86") Or ((Not proc64) And off = "x64") Then
        WScript.Echo "ADVERTENCIA: Mismatch de arquitectura (cscript vs Office: " & off & ")."
        WScript.Quit 3
    End If
End Sub
